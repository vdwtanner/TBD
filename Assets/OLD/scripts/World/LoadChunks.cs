...
public class LoadChunks : MonoBehaviour
{
    ...
    void FindChunksToLoad()
    {
        ...
        WorldPos playerPos = new WorldPos(
            Mathf.FloorToInt(transform.position.x / Config.Env.ChunkSize) * Config.Env.ChunkSize,
            Mathf.FloorToInt(transform.position.y / Config.Env.ChunkSize) * Config.Env.ChunkSize,
            Mathf.FloorToInt(transform.position.z / Config.Env.ChunkSize) * Config.Env.ChunkSize
            );
        ...
        if (updateList.Count == 0)
        {
            ...
            for (int i = 0; i < chunkPositions.Length; i++)
            {
				...
				//Build top down so that we can gen Sunlight
				for (int y = Config.Env.WorldMinY/Config.Env.ChunkSize; y <= Config.Env.WorldMaxY/Config.Env.ChunkSize; y++)
                {
                    ...
                    for (int x = newChunkPos.x - Config.Env.ChunkSize; x <= newChunkPos.x + Config.Env.ChunkSize; x += Config.Env.ChunkSize)
                    {
                        ...
                        {
                            buildList.Add(new WorldPos(x, y * Config.Env.ChunkSize, z));
                        }
                    }
                    updateList.Add(new WorldPos(newChunkPos.x, y * Config.Env.ChunkSize, newChunkPos.z));
                }
                return;
            }
        }
